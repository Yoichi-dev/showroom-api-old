...
exports.getEventUsers = (connection, id) => {
    return new Promise((resolve, reject) => {
        connection.query('select a.event_id as event_id, a.room_id as room_id, b.room_name as room_name, b.room_url_key as room_url_key, a.`rank` as juni from (select event_id, room_id, get_at, follower_num, gap, next_rank, point, `rank` from event_history where event_id = ? and get_at = (select max(get_at) from event_history where event_id = ?)) a left join users b on a.room_id = b.room_id order by a.`rank`',
            [id, id],
            (err, rows, fields) => {
                if (err) reject();
                resolve(rows);
